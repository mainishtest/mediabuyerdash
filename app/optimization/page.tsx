...
function emptyView(clients: Array<{ id: string; name: string }>, clientId: string | null, dateFrom = "", dateTo = "") {
  return (
    <OptimizationView
      clients={clients}
      clientId={clientId}
...
export default async function OptimizationPage({ searchParams }: PageProps) {
  ...
  try {
    ...
    console.error("[optimization] page error:", err);
    return emptyView(clients, clientId, dateFrom, dateTo);
  }
}
