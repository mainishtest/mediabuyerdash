...
/** Creative thumbnail for ad-level table rows */
function CreativeThumbnail({ creative }: { creative?: AdCreativeData }) {
  const src = creative?.thumbnailUrl ?? creative?.imageUrl;
  if (!src) {
    return (
      <div className="flex h-10 w-10 items-center justify-center rounded-lg border border-slate-700 bg-slate-800 text-xs text-slate-600">
        —
      </div>
    );
  }
  return (
    <div className="group relative">
      ...
      <img
        src={src}
        alt={creative?.creativeName ?? "Ad creative"}
        className="h-10 w-10 rounded-lg border border-slate-700 object-cover"
      />
      {/* Hover preview */}
      <div className="pointer-events-none absolute bottom-full left-0 z-30 mb-2 hidden rounded-xl border border-slate-700 bg-slate-900 shadow-2xl group-hover:block">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={creative?.imageUrl ?? src}
          alt={creative?.creativeName ?? "Ad creative preview"}
...
function EvaluationTable({
  ...
  /** Optional ROAS time-series keyed by entityId for inline sparklines */
  sparkData?:   Record<string, { date: string; value: number }[]>;
}) {
  ...
  const showThumbnails = !!adCreatives;
  const showSpark      = !!sparkData;
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-slate-800">
              {showThumbnails && <th className={TH}>Creative</th>}
              <th className={TH}>Entity</th>
              {showSpark && <th className={TH}>Trend</th>}
          ...
          <tbody className="divide-y divide-slate-800/60">
            {evaluations.map((ev) => (
              <EvaluationRow
                ...
                creative={adCreatives?.[ev.entityId]}
                showThumbnail={showThumbnails}
                sparkPoints={sparkData?.[ev.entityId]}
              />
            ))}
          </tbody>
        </table>
...
function EvaluationRow({
  ...
  sparkPoints?:   { date: string; value: number }[];
}) {
  ...
  // Color-coded ROAS: green >= goal, amber 50-99%, red < 50%
  const roasColor =
    roasMet  ? "text-emerald-400" :
    roasMiss ? (ev.actualRoas != null && ev.roasGoalValue > 0 && ev.actualRoas / ev.roasGoalValue >= 0.5
      ? "text-amber-400" : "text-rose-400") :
    "text-slate-500";

  // Color-coded CPA: green <= goal, amber 1-1.5x, red > 1.5x
  const cpaColor =
    cpaMet  ? "text-emerald-400" :
    cpaMiss ? (ev.actualCpa != null && ev.cpaGoalValue > 0 && ev.actualCpa / ev.cpaGoalValue <= 1.5
      ? "text-amber-400" : "text-rose-400") :
    "text-slate-500";
    ...
    >
      ...
      <td className={`${TD} max-w-[260px]`}>
        <div className="flex items-center gap-2">
          <span className="inline-flex shrink-0 items-center rounded border border-slate-700 bg-slate-800/60 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-slate-500">
            {ev.entityType === "adset" ? "AS" : ev.entityType === "campaign" ? "C" : "AD"}
          </span>
          <span className="truncate font-medium text-white" title={ev.entityName}>
            {ev.entityName}
          </span>
        </div>
      </td>
      {sparkPoints && (
        <td className={TD}>
          <MetricSparkline data={sparkPoints} height={24} width={80} />
        </td>
      )}
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
              ...
              <StatCard
                ...
                trend={
                  evaluatedCpa != null && totalCrmOrders > 0
                    ? {
                        direction: evaluatedCpa <= (totalSpend / Math.max(totalCrmOrders, 1) * 0.9) ? "down" as const :
                                   evaluatedCpa >= (totalSpend / Math.max(totalCrmOrders, 1) * 1.1) ? "up" as const : "flat" as const,
                        label: "vs period avg"
                      }
                    : undefined
                }
              ...
              <StatCard
                ...
                trend={
                  evaluatedRoas != null
                    ? {
                        direction: evaluatedRoas >= 2.0 ? "up" as const :
                                   evaluatedRoas < 1.0 ? "down" as const : "flat" as const,
                        label: evaluatedRoas >= 2.0 ? "Strong" :
                               evaluatedRoas >= 1.0 ? "Break-even" : "Below target"
                      }
                    : undefined
                }
              />
